...
class StreamingTransformer(StreamingModule[_TransformerState]):
    ...
    def __init__(
        ...
        rope_continuous: bool = False,
        rope_reset_on_new_file: bool = True,
        device=None,
        dtype=None,
        **kwargs,
    ):
        ...
        # Continuous RoPE configuration (for training mode)
        self.rope_continuous = rope_continuous
        self.rope_reset_on_new_file = rope_reset_on_new_file
        self._training_offsets: tp.Dict[str, int] = {}  # file_id -> current_offset
        self._last_file_id: tp.Optional[str] = None
    ...
    def forward(self, x: torch.Tensor, file_id: tp.Optional[str] = None, *args, **kwargs):
        B, T, C = x.shape

        dtype_input = x.dtype
        state = self._streaming_state
        
        # DEBUG: Log once per 100 forward passes
        if not hasattr(self, '_forward_count'):
            self._forward_count = 0
            self._logged_rope_config = False
        self._forward_count += 1
        
        if state is None:
            # Training mode (non-streaming)
            if self.rope_continuous and file_id is not None:
                # DEBUG: Log first time continuous RoPE is used
                if not self._logged_rope_config:
                    import logging
                    logger = logging.getLogger(__name__)
                    logger.info(f"[RoPE-DEBUG] CONTINUOUS RoPE ACTIVE: file_id={file_id}, rope_continuous={self.rope_continuous}")
                    self._logged_rope_config = True
                
                # Continuous RoPE: track per-file offsets
                if file_id != self._last_file_id:
                    if self.rope_reset_on_new_file or file_id not in self._training_offsets:
                        self._training_offsets[file_id] = 0
                    self._last_file_id = file_id
                
                current_offset = self._training_offsets.get(file_id, 0)
                offsets = torch.full((B,), current_offset, dtype=torch.long, device=x.device)
                
                # Update offset for next forward pass
                self._training_offsets[file_id] = current_offset + T
                
                # DEBUG: Log offset every 10 forward passes to verify incrementing
                if self._forward_count % 10 == 0:
                    import logging
                    logger = logging.getLogger(__name__)
                    logger.info(f"[RoPE-DEBUG] OFFSET UPDATE: file_id={file_id[:50]}..., current_offset={current_offset}, T={T}, new_offset={current_offset + T}, total_files={len(self._training_offsets)}")
            else:
                # Default behavior: always start at 0
                if not self._logged_rope_config and self._forward_count == 1:
                    import logging
                    logger = logging.getLogger(__name__)
                    logger.info(f"[RoPE-DEBUG] STANDARD RoPE: rope_continuous={self.rope_continuous}, file_id={file_id}")
                    self._logged_rope_config = True
                offsets = torch.zeros(1, dtype=torch.long, device=x.device)
        else:
            # Streaming mode: use state offsets as before
...
class ProjectedTransformer(StreamingContainer):
    ...
    def forward(self, x, file_id: tp.Optional[str] = None, *args, **kwargs):
        if self.conv_layout:
            x = x.transpose(1, 2)
        if self.input_proj is not None:
            x = self.input_proj(x)
        z = self.transformer(x, file_id=file_id, *args, **kwargs)
        ys = []
