...
def main():
    ...
    parser.add_argument(
        "--lora-weight", type=str, help="Path to a local LoRA weights file."
    )
    parser.add_argument(
        "--hf-repo",
        type=str,
    ...
    if hasattr(lm, 'transformer') and hasattr(lm.transformer, 'rope') and lm.transformer.rope is not None:
        ...
        log("info", "RoPE fix applied: max_period 10,000 → 100,000")
    ...
    if hasattr(lm, 'transformer') and hasattr(lm.transformer, 'layers'):
        ...
        if old_context is not None:
            log("info", f"KV cache context: {old_context} → {new_context}")
