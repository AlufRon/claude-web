...
class LMGen(StreamingModule[_LMGenState]):
    ...
    def step_with_logits(
        ...
        depformer_replace_tokens: torch.Tensor | None = None,
        teacher_forcing_tokens: torch.Tensor | None = None,
    ) -> dict[str, torch.Tensor] | None:
        """
        Like step() but also returns logits for evaluation purposes.
        
        This is useful for computing loss during evaluation without sampling.
        Does not affect normal inference - completely separate method.
        
        Args:
            input_tokens: Input tokens [B, K, 1]
            depformer_replace_tokens: Optional replacement tokens [B, dep_q, 1] for Moshi audio stream.
                                     This enables autoregressive evaluation on Moshi's own stream
                                     (e.g., for LibriLight) instead of user->moshi response mode.
            teacher_forcing_tokens: Optional ground truth tokens [B, dep_q] for depformer teacher forcing.
                                   If provided, uses these instead of sampling during depformer.
                                   This prevents error accumulation for loss computation.
        
        Returns:
            Dictionary with:
                - 'tokens': Sampled output tokens [B, K, 1] (same as step())
                - 'text_logits': Text logits [B, 1, 1, text_vocab_size]
                - 'audio_logits': Audio logits [B, dep_q, vocab_size]
            Or None if not ready to output yet.
        """
        # Reuse existing _step() infrastructure with depformer_replace_tokens
        state = self._streaming_state
        if state is None:
            raise RuntimeError(
                "You should wrap those calls with a `with lm_gen.streaming(): ...`."
        ...
        # Get the internal step output (with optional depformer replacement)
        out = self._step(input_tokens, depformer_replace_tokens)
        if out is None:
            return None
        
        tokens, transformer_out = out
        
        # We need to reconstruct logits from the streaming state
        # Text logits were computed in _step() but not returned
        # We'll compute them again (minimal overhead)
        
        lm_model = self.lm_model
        B = tokens.shape[0]
        
        # Get text logits from the last forward pass
        # This requires re-computing, but it's the cleanest approach
        # that doesn't modify _step() behavior
        
        # Actually, we can access the cached text_logits from the hook if set
        # For now, we'll store them in a simple way:
        # Just return what we can reconstruct from transformer_out
        
        # Get text logits by calling forward_text on current state
        # Note: This is a simplified version - for full accuracy we'd need
        # to cache text_logits in _step(), but that would modify existing code
        
        # For audio logits, we can compute from transformer_out
        text_token = tokens[:, 0, 0]  # [B]
        
        # Compute audio logits using depformer WITH TEACHER FORCING if available
        audio_logits_list = []
        with lm_model.depformer.streaming(B):
            ...
            for cb_index in range(lm_model.dep_q):
                ...
                # logits is [B, 1, 1, vocab_size], squeeze to [B, vocab_size]
                logits_squeezed = logits.squeeze(1).squeeze(1)  # [B, vocab_size]
                audio_logits_list.append(logits_squeezed)
                
                # Use teacher forcing if available, otherwise sample
                if teacher_forcing_tokens is not None:
                    # Use ground truth token for next iteration (teacher forcing)
                    prev_token = teacher_forcing_tokens[:, cb_index]
                else:
                    # Sample for next iteration (autoregressive)
        ...
        # Stack audio logits: [B, dep_q, vocab_size]
        audio_logits = torch.stack(audio_logits_list, dim=1)  # [B, dep_q, vocab_size]
        
        return {
            'tokens': tokens,
            'text_logits': None,  # Would need to modify _step() to cache this
