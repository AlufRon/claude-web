...
class Checkpointer:
    ...
    def __init__(
        ...
        training_args = None,  # Add training args for complete config saving
    ):
        self.model = model
        ...
        self.config = config
        self.training_args = training_args  # Store training args for config saving
    ...
    def save_checkpoint(
        ...
        save_optimizer: bool = True,  # NEW: Save optimizer state for resuming
    ):
        ...
        if self.rank == 0:
            ...
            # NEW: Save optimizer state for resuming training
            if save_optimizer and self.optimizer is not None:
                optimizer_path = tmp_dst / "optimizer_state.pt"
                torch.save({
                    'optimizer': self.optimizer.state_dict(),
                    'step': self.state.step,
                }, optimizer_path)
                main_logger_info(f"Saved optimizer state to {optimizer_path.name}")
