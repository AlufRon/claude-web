...
class Sample:
    ...
    file_id: str | None = None  # Track source file for continuous RoPE
...
class Batch:
    ...
    def collate(cls, batch: list[Sample]) -> "Batch":
        ...
        condition_attrs = None if batch[0].condition_attributes is None else [b.condition_attributes for b in batch]
        file_id = batch[0].file_id if batch[0].file_id is not None else None
        chunk_index = batch[0].chunk_index if batch[0].chunk_index is not None else None
        return Batch(codes, condition_attrs, file_id, chunk_index)
...
class InterleavedTokenizer:
    ...
    def __call__(self, wav: np.ndarray, start_sec: float, path: str) -> Sample | None:
        with torch.no_grad():
            try:
                ...
                print(f"⚠️  Skipping problematic audio file {path}: {e}")
                return None
            ...
            try:
                ...
                print(f"⚠️  Skipping file with missing/invalid JSON {path}: {e}")
                return None
            ...
            return Sample(codes, data.get("text_conditions", None), file_id=file_id, chunk_index=chunk_index)
