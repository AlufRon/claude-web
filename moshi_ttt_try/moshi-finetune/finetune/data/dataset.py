...
def is_librilight_dataset(pretrain_data: str) -> bool:
    """
    Check if dataset is LibriLight (audio-only).

    Returns True if path contains 'librilight' (case-insensitive).
    """
    return 'librilight' in pretrain_data.lower()
...
def build_dataset(
    ...
    # Check if this is LibriLight dataset (audio-only, no text)
    is_librilight = is_librilight_dataset(pretrain_data)

    if is_librilight:
        main_logger_info("=" * 80)
        main_logger_info("📢 LIBRILIGHT DATASET DETECTED - Using audio-only mode")
        main_logger_info("=" * 80)
        main_logger_info("   Text stream will be zero-filled (no text training)")
        main_logger_info("   Mono audio will be duplicated to stereo")
        main_logger_info("=" * 80)

        # Import LibriLight components
        from .librilight_interleaver import LibriLightInterleaver, LibriLightTokenizer

        # Create audio-only tokenizer
        # Use text_padding (token ID 3) instead of zero_padding (token ID -1)
        # This allows the text loss to be computed (but heavily downweighted)
        # rather than causing division by zero with all-masked text stream
        librilight_interleaver = LibriLightInterleaver(
            text_padding=instruct_tokenizer.interleaver.text_padding
        )
        librilight_tokenizer = LibriLightTokenizer(
            mimi=instruct_tokenizer.mimi,
            interleaver=librilight_interleaver,
            duration_sec=instruct_tokenizer.duration_sec
        )
        # Use LibriLight tokenizer instead of standard tokenizer
        actual_tokenizer = librilight_tokenizer
    else:
        # Use standard tokenizer for normal datasets (DailyTalk, etc.)
        actual_tokenizer = instruct_tokenizer
    ...
    dataset_iterators = [
        get_dataset_iterator(
            source,
            instruct_tokenizer=actual_tokenizer,  # Use detected tokenizer
            rank=rank,
